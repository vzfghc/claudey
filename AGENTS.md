...
- Repository protection should use rulesets: a non-bypassable main integrity ruleset requires pull requests, merge queue, required checks, and blocks direct/force pushes to `main`; a separate review ruleset may allow `vzfghc`/admins to bypass review only.
...
- **Shared utilities**: Put shared Anthropic protocol logic in neutral `src/claudey/core/anthropic/` modules. Do not have one provider import from another provider's utils.
...
- `src/claudey/api/`, `src/claudey/cli/`, `src/claudey/config/`, `src/claudey/core/`, `src/claudey/messaging/`, `src/claudey/providers/`
- `src/claudey/application/`
- `.env.example`
