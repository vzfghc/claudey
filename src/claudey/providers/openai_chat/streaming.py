...
from claudey.providers.admission import ProviderAttempt, ProviderRetrySession
...
from claudey.providers.http import close_provider_stream, maybe_await_aclose
...
from .streaming_helpers import content_events, reasoning_events, tool_call_events
from .streaming_types import OpenAIChatProviderProtocol
...
def _held_events(recovery: RecoveryController, events: Iterator[str]) -> Iterator[str]:
    """Buffer events until recovery permits their public emission."""
    for event in events:
...
class OpenAIChatStreamRunner:
    ...
    def __init__(
        self,
        provider: OpenAIChatProviderProtocol,
        *,
    ...
    async def run(self) -> AsyncIterator[str]:
        ...
        while True:
            ...
            if not ledger.message_started:
                for event in _held_events(recovery, iter((ledger.message_start(),))):
                    yield event
            ...
            try:
                ...
                async for chunk in stream:
                    ...
                    for event in _held_events(
                        recovery,
                        reasoning_events(
                            self._provider._profile,
                            self._provider._handle_extra_reasoning,
                            delta,
                            ledger,
