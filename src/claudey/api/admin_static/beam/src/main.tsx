// Provider-beam micro-frontend entry. Renders the real magicui AnimatedBeam
// diagram (providers → claudey gateway → your agent) inside the shadow root
// of a plain <div id="providerBeamMount">, keeping React + motion + styles
// fully isolated from the vanilla admin UI. The vanilla admin.js calls
// window.ClaudeyBeam.mount() with the live provider list after /admin/api/config.
//
// The canvas is pan/zoom (drag the background, wheel/buttons to zoom) and every
// provider node is individually draggable: grab a node and drop it anywhere in
// the canvas. Zoom scales the layout via the --bs CSS custom property, so the
// beam paths are re-measured by the component's ResizeObserver at every zoom
// level; node drags bump a measureKey that forces the same re-measure so beams
// stay attached while a node is being moved.
import { createRef, useCallback, useEffect, useRef, useState, type CSSProperties, type RefObject } from "react";
import { createRoot, type Root } from "react-dom/client";

import { AnimatedBeam } from "./components/animated-beam";
import beamCss from "./beam.css";
...
function ProviderBeamDiagram({
  ...
  const [positions, setPositions] = useState<Record<string, ProviderPos>>(() => {
    ...
    options.providers.forEach((provider, index) => {
      const angle = (index / Math.max(options.providers.length, 1)) * Math.PI * 2 - Math.PI / 2;
  ...
  // Zoom faster than the original 5s pulse and desync the per-provider beams
  // so light is almost always moving somewhere on the diagram.
  const beamMotion = {
  ...
  const zoomButtons = (factor: number) => {
    ...
    setView((current) => {
      ...
      const k = s / current.s;
      return { x: cx - (cx - current.x) * k, y: cy - (cy - current.y) * k, s };
    });
  };
      ...
      >
        <div className="beam-diagram" ref={diagramRef} style={diagramStyle}>
          ...
          >
            ...
            <span className="beam-label">{options.gateway?.name ?? "claudey"}</span>
          </div>
          <div
          ...
          >
            ...
            <span className="beam-label">{options.client?.name ?? "Your agent"}</span>
          </div>
          {providers.map((provider, index) => (
            <AnimatedBeam
...
const mounts = new Map<string, BeamMount>();

function reducedMotionActive(): boolean {
  return window.matchMedia("(prefers-reduced-motion: reduce)").matches;
}

function ensureMounted(containerId: string): HTMLElement | null {
  const container = document.getElementById(containerId);
  if (!container) return null;
  const existing = mounts.get(containerId);
  ...
  host.className = "beam-root";
  shadow.appendChild(host);
  mounts.set(containerId, { root: createRoot(host) });
  return container;
}

export function mountBeam(containerId: string, options: BeamOptions): void {
  const container = ensureMounted(containerId);
  if (!container) return;
  const mount = mounts.get(containerId);
  if (!mount) return;
  mount.root.render(
    <ProviderBeamDiagram
      options={options}
...
export function unmountBeam(containerId: string): void {
  const mount = mounts.get(containerId);
  if (!mount) return;
  mount.root.unmount();
  mounts.delete(containerId);
  const container = document.getElementById(containerId);
...
declare global {
  interface Window {
    ClaudeyBeam: {
      mount: typeof mountBeam;
      unmount: typeof unmountBeam;
    };
  }
}
