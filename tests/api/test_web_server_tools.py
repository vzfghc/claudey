...
import claudey.api.web_tools.constants as web_tool_constants
from claudey.api.handlers import MessagesHandler
from claudey.api.web_tools import egress as web_egress
from claudey.api.web_tools.egress import (
    WebFetchEgressPolicy,
    WebFetchEgressViolation,
    enforce_web_fetch_egress,
)
from claudey.api.web_tools.outbound import (
...
def test_web_tool_user_agent_reports_installed_package_version() -> None:
    assert {
        "User-Agent": (f"Mozilla/5.0 compatible; claudey/{package_version()}")
...
async def test_service_rejects_forced_server_tool_when_local_handler_is_disabled(
    ...
    """Every provider needs Claudey's local handler for forced server tools."""
    settings = Settings()
...
async def test_run_web_fetch_follows_redirect_when_each_hop_is_allowed():
    ...
    with patch("claudey.api.web_tools.outbound.ClientSession", return_value=client_cm):
...
async def test_run_web_fetch_redirect_to_blocked_host_raises():
    ...
    with (
        patch(
            "claudey.api.web_tools.outbound.ClientSession",
...
async def test_run_web_fetch_excess_redirects_raises():
    ...
    with (
        patch("claudey.api.web_tools.constants._MAX_WEB_FETCH_REDIRECTS", 1),
        patch(
            "claudey.api.web_tools.outbound.ClientSession",
            return_value=client_cm,
        ),
...
async def test_streams_web_search_server_tool_result(monkeypatch):
    ...
    monkeypatch.setattr("claudey.api.web_tools.outbound._run_web_search", fake_search)
    request = MessagesRequest(
        model="claude-haiku-4-5-20251001",
        max_tokens=100,
        messages=[
...
async def test_forced_web_fetch_ignores_stale_url_from_prior_user_turns(monkeypatch):
    ...
    monkeypatch.setattr("claudey.api.web_tools.outbound._run_web_fetch", fake_fetch)
    request = MessagesRequest(
        model="claude-haiku-4-5-20251001",
        max_tokens=100,
        messages=[
...
async def test_streams_web_fetch_error_summary_generic_by_default(monkeypatch):
    ...
    monkeypatch.setattr("claudey.api.web_tools.outbound._run_web_fetch", boom)
    request = MessagesRequest(
        model="claude-haiku-4-5-20251001",
        max_tokens=100,
        messages=[
    ...
    with patch("claudey.api.web_tools.outbound.logger.warning") as log_warn:
        raw = "".join(
            [
                event
                async for event in stream_web_server_tool_response(
