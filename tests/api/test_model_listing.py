...
def test_models_list_includes_configured_refs_cached_provider_models_and_aliases():
    ...
    assert ids[:6] == [
        ...
        "claude-3-claudey-no-thinking/deepseek/deepseek-chat",
        "anthropic/open_router/anthropic/claude-opus",
        "claude-3-claudey-no-thinking/open_router/anthropic/claude-opus",
        "anthropic/open_router/meta/llama-3.3",
        "claude-3-claudey-no-thinking/open_router/meta/llama-3.3",
    ]
    ...
    assert (
        display_names["claude-3-claudey-no-thinking/open_router/meta/llama-3.3"]
...
def test_models_list_uses_thinking_metadata_for_cached_models():
    ...
    assert "claude-3-claudey-no-thinking/open_router/reasoning-model" in ids
...
def test_models_list_uses_cached_metadata_for_configured_refs():
    ...
    ids = [item["id"] for item in response.json()["data"]]
    assert "anthropic/open_router/plain-model" not in ids
    assert ids[0] == "claude-3-claudey-no-thinking/open_router/plain-model"
...
def test_models_list_includes_cached_wafer_models():
    ...
    assert "claude-3-claudey-no-thinking/wafer/DeepSeek-V4-Pro" in ids
    assert "anthropic/wafer/MiniMax-M2.7" in ids
    assert "claude-3-claudey-no-thinking/wafer/MiniMax-M2.7" in ids
