...
from claudey.runtime.application import (
    ApplicationRuntime,
    startup_failure_message,
    warn_if_process_auth_token,
)
from claudey.runtime.asgi import RuntimeASGIApp
from claudey.runtime.bootstrap import _create_transcriber, build_asgi_app
...
def test_warn_if_process_auth_token_logs_warning(monkeypatch):
    ...
    with patch("claudey.runtime.application.logger.warning") as warning:
...
def test_application_error_handler_does_not_log_error_message():
    ...
    with patch("claudey.api.app.logger.error") as log_error:
...
def test_bootstrap_configures_default_log_and_publishes_only_services(tmp_path):
    ...
    with (
        patch(
            "claudey.runtime.bootstrap.server_log_path",
            return_value=log_path,
        ),
        patch("claudey.runtime.bootstrap.configure_logging") as configure,
    ):
...
def test_bootstrap_wires_the_codex_catalog_publisher() -> None:
    ...
    with (
        patch("claudey.runtime.bootstrap.configure_logging"),
        patch(
            "claudey.runtime.bootstrap.CodexModelCatalogPublisher",
...
def test_bootstrap_honors_process_log_file_override(monkeypatch, tmp_path):
    ...
    with patch("claudey.runtime.bootstrap.configure_logging") as configure:
...
async def test_bootstrap_constructs_isolated_runtime_resource_graphs() -> None:
    ...
    with patch("claudey.runtime.bootstrap.configure_logging"):
