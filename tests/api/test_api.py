...
def test_create_message_pre_start_provider_error_returns_terminal_json(
    ...
    with (
        patch("claudey.api.response_streams.trace_event") as trace,
        patch("claudey.application.execution.trace_event") as execution_trace,
    ):
        response = client.post("/v1/messages", json=payload)
    ...
    route_trace = next(
        ...
        if call.kwargs.get("event") == "claudey.api.route.resolved"
    )
    assert route_trace["request_id"] == request_id
    terminal_trace = next(
        call.kwargs
        for call in trace.call_args_list
        if call.kwargs.get("event") == "claudey.api.response.terminal_execution_error"
    )
