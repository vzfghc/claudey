...
class TestBuildRequestBody:
    ...
    def test_grep_schema_type_parameter_is_aliased_without_mutating_request(self, req):
        ...
        tool_schema["properties"]["_hans_arg_type"] = {
            "type": "string",
            "description": "Existing safe property that collides with the alias",
        }
        tool_schema["required"] = ["pattern", "-A", "_hans_arg_type"]
        ...
        assert (
            properties["_hans_arg_type"]
            == original_schema["properties"]["_hans_arg_type"]
        )
        assert aliases == {"_hans_arg_type_2": "type"}
        assert properties["_hans_arg_type_2"] == original_schema["properties"]["type"]
        assert "-A" in parameters["required"]
        assert "_hans_arg_type" in parameters["required"]
    ...
    def test_nested_schema_keyword_properties_are_aliased_without_mutating_request(
        ...
        assert parent_properties["_hans_arg_type"] == {
            "type": "string",
            "enum": ["page_id"],
        }
        assert parent["required"] == ["_hans_arg_type", "id"]
        assert aliases == {"_hans_arg_type": "type"}
        assert tool_schema == original_schema

    def test_private_alias_metadata_is_stripped_without_mutating_body(self):
        body = {
            "model": "test",
            NIM_TOOL_ARGUMENT_ALIASES_KEY: {"Grep": {"_hans_arg_A": "-A"}},
        }

        upstream_body = body_without_nim_tool_argument_aliases(body)

        assert NIM_TOOL_ARGUMENT_ALIASES_KEY not in upstream_body
        assert body[NIM_TOOL_ARGUMENT_ALIASES_KEY] == {"Grep": {"_hans_arg_A": "-A"}}
