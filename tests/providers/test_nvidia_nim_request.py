...
from claudey.core.anthropic import set_if_not_none
from claudey.core.anthropic.models import MessagesRequest, Tool
from claudey.core.reasoning import ReasoningEffort, ReasoningPolicy
from claudey.providers.nvidia_nim.request_options import (
    _set_extra,
)
from claudey.providers.nvidia_nim.request_options import (
    build_nim_request_body as build_request_body,
)
from claudey.providers.nvidia_nim.retry import (
    clone_body_without_chat_template,
    clone_body_without_reasoning_content,
)
from claudey.providers.nvidia_nim.tool_schema import (
...
class TestBuildRequestBody:
    ...
    def test_grep_schema_type_parameter_is_aliased_without_mutating_request(self, req):
        ...
        tool_schema["properties"]["_claudey_arg_type"] = {
            "type": "string",
            "description": "Existing safe property that collides with the alias",
        }
        tool_schema["required"] = ["pattern", "-A", "_claudey_arg_type"]
        ...
        assert (
            properties["_claudey_arg_type"]
            == original_schema["properties"]["_claudey_arg_type"]
        )
        assert aliases == {"_claudey_arg_type_2": "type"}
        assert (
            properties["_claudey_arg_type_2"] == original_schema["properties"]["type"]
        )
        assert "-A" in parameters["required"]
        assert "_claudey_arg_type" in parameters["required"]
    ...
    def test_reported_long_tool_name_uses_generic_alias_after_nim_repairs(self, req):
        ...
        assert body[NIM_TOOL_ARGUMENT_ALIASES_KEY] == {
            long_name: {"_claudey_arg_type": "type"}
        }
        assert (
            NIM_TOOL_ARGUMENT_ALIASES_KEY
    ...
    def test_nested_schema_keyword_properties_are_aliased_without_mutating_request(
        ...
        assert parent_properties["_claudey_arg_type"] == {
            "type": "string",
            "enum": ["page_id"],
        }
        assert parent["required"] == ["_claudey_arg_type", "id"]
        assert aliases == {"_claudey_arg_type": "type"}
        assert tool_schema == original_schema

    def test_private_alias_metadata_is_stripped_without_mutating_body(self):
        body = {
            "model": "test",
            NIM_TOOL_ARGUMENT_ALIASES_KEY: {"Grep": {"_claudey_arg_A": "-A"}},
        }

        upstream_body = body_without_nim_tool_argument_aliases(body)

        assert NIM_TOOL_ARGUMENT_ALIASES_KEY not in upstream_body
        assert body[NIM_TOOL_ARGUMENT_ALIASES_KEY] == {"Grep": {"_claudey_arg_A": "-A"}}
        assert nim_tool_argument_aliases_from_body(body) == {
            "Grep": {"_claudey_arg_A": "-A"}
        }
