...
from claudey.providers.runtime import (
    ProviderRuntime,
...
def test_importing_runtime_does_not_eager_load_other_adapters() -> None:
    ...
    code = (
        ...
        "assert 'claudey.providers.open_router' not in sys.modules\n"
    )
    proc = subprocess.run(
...
def test_create_provider_instantiates_each_builtin():
    ...
    with (
        patch("claudey.providers.openai_chat.provider.AsyncOpenAI"),
        patch("httpx.AsyncClient"),
        patch(
            "claudey.providers.runtime.factory.ProviderAdmissionController",
