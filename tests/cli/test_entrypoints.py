...
def test_hans_server_reports_version_without_side_effects(
    ...
    assert capsys.readouterr() == ("claudey 9.8.7\n", "")
...
def test_version_entrypoint_does_not_import_command_runtime() -> None:
    script = "\n".join(
        (
            "import json",
            "import sys",
            "from claudey.cli.entrypoints import serve",
            "serve(['--version'])",
...
def test_non_version_entrypoint_delegates_to_server_command() -> None:
    from claudey.cli import commands, entrypoints
...
def test_launch_claude_passes_args_and_child_env(
    ...
    from claudey.cli.launchers.claude import launch
    ...
    with (
        patch("claudey.cli.launchers.claude.get_settings", return_value=settings),
        patch("claudey.cli.launchers.claude.preflight_proxy", return_value=None),
        patch(
            "claudey.cli.launchers.common.shutil.which",
            return_value="resolved-claude.cmd",
        ),
        patch("claudey.cli.launchers.common.subprocess.Popen") as popen,
        patch("claudey.cli.launchers.common.register_pid") as register_pid,
        patch("claudey.cli.launchers.common.unregister_pid") as unregister_pid,
...
def test_launch_codex_passes_responses_config_and_child_env(
    ...
    from claudey.cli.launchers.codex import launch
    ...
    with (
        patch("claudey.cli.launchers.codex.get_settings", return_value=settings),
        patch("claudey.cli.launchers.codex.preflight_proxy", return_value=None),
        patch(
            "claudey.cli.launchers.common.shutil.which",
            return_value="resolved-codex.cmd",
        ),
        patch(
            "claudey.cli.launchers.codex.codex_model_catalog_path",
            return_value=catalog_path,
        ),
        patch(
            "claudey.cli.launchers.codex.open_local_request",
...
def test_launch_codex_catalog_failure_warns_and_continues(
    ...
    with (
        ...
        patch("claudey.cli.launchers.common.subprocess.Popen") as popen,
        patch("claudey.cli.launchers.common.register_pid"),
        patch("claudey.cli.launchers.common.unregister_pid"),
        pytest.raises(SystemExit) as exc_info,
    ):
        process = popen.return_value
        process.pid = 12345
...
def test_pi_launcher_builds_scoped_session_command_and_proxy_env(
    ...
    from claudey.cli.launchers.pi import (
...
def test_launch_pi_registers_bundled_extension_for_sessions(
    ...
    with (
        patch("claudey.cli.launchers.pi.get_settings", return_value=settings),
        patch("claudey.cli.launchers.pi.preflight_proxy", return_value=None),
        patch(
            "claudey.cli.launchers.pi.pi_extension_path",
        ...
        patch(
            "claudey.cli.launchers.pi.pi_binary_is_compatible",
            return_value=True,
        ),
        patch("claudey.cli.launchers.common.subprocess.Popen") as popen,
        patch("claudey.cli.launchers.common.register_pid"),
...
def test_launch_pi_passes_management_commands_through_without_proxy(
    ...
    with (
        patch("claudey.cli.launchers.pi.get_settings") as get_settings,
        patch("claudey.cli.launchers.pi.preflight_proxy") as preflight,
        patch(
            "claudey.cli.launchers.common.shutil.which",
...
def test_pi_install_hints_use_official_platform_installers() -> None:
    from claudey.cli.launchers.pi import pi_install_hint
...
def test_pi_binary_compatibility_requires_both_launcher_capabilities(
    ...
    from claudey.cli.launchers.pi import pi_binary_is_compatible

    with patch(
        "claudey.cli.launchers.pi.subprocess.run",
...
def test_launch_claude_keyboard_interrupt_kills_child_tree() -> None:
    ...
    with (
        ...
        patch("claudey.cli.launchers.common.subprocess.Popen") as popen,
        patch("claudey.cli.launchers.common.register_pid"),
        patch("claudey.cli.launchers.common.kill_pid_tree_best_effort") as kill_tree,
        patch("claudey.cli.launchers.common.unregister_pid") as unregister_pid,
...
def test_launch_claude_exits_when_command_cannot_be_resolved(
    ...
    with (
        patch("claudey.cli.launchers.claude.get_settings", return_value=settings),
        patch("claudey.cli.launchers.claude.preflight_proxy", return_value=None),
        patch("claudey.cli.launchers.common.shutil.which", return_value=None),
        patch("claudey.cli.launchers.common.subprocess.Popen") as popen,
        pytest.raises(SystemExit) as exc_info,
    ):
        launch([])
...
def test_launch_claude_unreachable_proxy_exits_with_hint(
    ...
    with (
        patch("claudey.cli.launchers.claude.get_settings", return_value=settings),
        patch(
            "claudey.cli.launchers.claude.preflight_proxy",
