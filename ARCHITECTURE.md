...
This document is a maintainer-oriented map of Claudey. It explains the
...
Claudey is a local proxy for agent clients. It accepts Anthropic
...
- [src/claudey/cli/](src/claudey/cli/) owns console entrypoints, client CLI launchers, process/session
  management, and client adapter contracts.
- [src/claudey/config/](src/claudey/config/) owns settings, provider metadata, filesystem paths,
...
- [src/claudey/messaging/](src/claudey/messaging/) owns optional platform adapters, incoming message
  ...
  support.
- [src/claudey/providers/](src/claudey/providers/) owns provider construction, the shared OpenAI-chat
...
- [src/claudey/runtime/](src/claudey/runtime/) is the process composition root. It owns application
...
`claudey.cli.entrypoints` imports
`claudey.runtime.bootstrap` because the installed server executable
...
provider execution belong in [src/claudey/application/](src/claudey/application/). Routes use core schemas
...
Claudey optimizes for installed user workflows, not internal compatibility. The
...
- [api/handlers/](src/claudey/api/handlers/) owns customer-facing API product flows:
  ...
  [application/execution.py](src/claudey/application/execution.py) only for shared
  provider resolution, preflight, tracing, token counting, and streaming.
- [providers/openai_chat/](src/claudey/providers/openai_chat/) owns the common upstream provider
  ...
  protocol rules belong in [src/claudey/core/](src/claudey/core/).
- [messaging/workflow.py](src/claudey/messaging/workflow.py) coordinates messaging runtime
...
- [config/admin/](src/claudey/config/admin/) owns Admin UI config behavior. Keep
...
installer owns the Claudey desktop and Start-menu shortcuts; on macOS it owns the
...
artifacts, the Claudey uv tool, and the managed `~/.Claudey/` tree from
[config/paths.py](src/claudey/config/paths.py); they do not remove
...
[cli/entrypoints.py](src/claudey/cli/entrypoints.py) starts the FastAPI server with Uvicorn.
...
[cli/desktop.py](src/claudey/cli/desktop.py) owns the platform-neutral
...
configures logging, constructs the runtime owners and the configured voice
  ...
  the consumer-owned ports in [application/ports.py](src/claudey/application/ports.py); Admin operations retain
  their inbound-adapter port in [api/ports.py](src/claudey/api/ports.py).

[api/app.py](src/claudey/api/app.py) registers routers and exception
...
[runtime/application.py](src/claudey/runtime/application.py) owns process startup and shutdown, optional messaging,
...
[runtime/asgi.py](src/claudey/runtime/asgi.py) drives that owner from ASGI lifespan messages and preserves
the concise startup-failure contract.

[runtime/provider_manager.py](src/claudey/runtime/provider_manager.py) is the only owner that constructs, publishes,
...
streaming responses bind it to Claudey's response owner, which first closes the
...
[config/settings.py](src/claudey/config/settings.py) owns the flat Pydantic Settings schema:
...
in [config/env_files.py](src/claudey/config/env_files.py) and uses this order:
...
source detection for startup warnings also belongs to `src/claudey/config/env_files.py`.

[config/paths.py](src/claudey/config/paths.py) defines managed paths:

- config directory: `~/.Claudey`;
- managed env file: `~/.Claudey/.env`;
- generated Codex model catalog: `~/.Claudey/codex-model-catalog.json`;
- messaging state directory: `~/.Claudey/agent_workspace`;
- server log: `~/.Claudey/logs/server.log`.
...
[config/admin/](src/claudey/config/admin/) owns the Admin UI config manifest and
managed env writes. Provider credential, configurable base URL, proxy, and display-name
metadata is generated from [config/provider_catalog.py](src/claudey/config/provider_catalog.py);
...
separate modules. [api/admin_routes.py](src/claudey/api/admin_routes.py) exposes local-only
...
packaged as a [src/claudey/config/](src/claudey/config/) resource for Admin UI defaults;
runtime settings do not read it as a live config file. The Admin UI creates and
atomically replaces `~/.Claudey/.env` when configuration is applied; server startup
...
[api/routes.py](src/claudey/api/routes.py) exposes the public proxy routes:
...
Admin routes live beside these in [api/admin_routes.py](src/claudey/api/admin_routes.py).

Authentication is handled by `require_proxy_auth()` in
[api/dependencies.py](src/claudey/api/dependencies.py). If `ANTHROPIC_AUTH_TOKEN` is blank,
proxy auth is disabled. Otherwise Claudey accepts exactly `Authorization: Bearer
...
opaque Claudey request ID before routing, places it in log context and request state,
...
[api/handlers/](src/claudey/api/handlers/) owns the public API product flows.
...
[application/execution.py](src/claudey/application/execution.py). `ProviderExecutor` resolves the narrow
...
response. Once Claudey has finalized the failure, the response includes
`x-should-retry: false` so Claudey retains ownership of upstream retry/recovery
...
[src/claudey/core/openai_responses/adapter.py](src/claudey/core/openai_responses/adapter.py). The adapter
...
[application/routing.py](src/claudey/application/routing.py) resolves incoming client model names.
It supports two forms:

- Direct provider model refs such as `nvidia_nim/nvidia/model-name`.
- Gateway model IDs decoded by [core/gateway_model_ids.py](src/claudey/core/gateway_model_ids.py).
...
[application/reasoning.py](src/claudey/application/reasoning.py) then
...
[providers/runtime/discovery.py](src/claudey/providers/runtime/discovery.py)
...
[runtime/codex_catalog.py](src/claudey/runtime/codex_catalog.py) is the
...
`~/.Claudey/codex-model-catalog.json` without making a loopback HTTP request.
...
[config/provider_catalog.py](src/claudey/config/provider_catalog.py). Each
...
[providers/runtime/](src/claudey/providers/runtime/) owns construction details for one
...
cleanup. [providers/runtime/factory.py](src/claudey/providers/runtime/factory.py)
...
[providers/admission.py](src/claudey/providers/admission.py) owns the
...
[providers/base.py](src/claudey/providers/base.py) defines provider-internal construction and lifecycle contracts:
...
[providers/openai_codex/](src/claudey/providers/openai_codex/) owns
ChatGPT subscription authentication and the Codex backend's OpenAI Responses
transport. Its process-lifetime auth manager owns Claudey's credential file,
...
[core/openai_responses/](src/claudey/core/openai_responses/), which
...
combining it with Claudey reasoning controls fails during deterministic preflight.
...
API-key endpoint; [providers/vertex/](src/claudey/providers/vertex/)
...
[core/anthropic/](src/claudey/core/anthropic/) owns reversible
...
to Kimi's named efforts and identifies Claudey through the upstream user agent.
...
[providers/mistral/reasoning.py](src/claudey/providers/mistral/reasoning.py), including its
...
downgrade: if an upstream NIM deployment rejects explicit budget control, Claudey
...
[core/reasoning.py](src/claudey/core/reasoning.py) owns the immutable,
...
effort through Claudey's single product scale: `minimal`/`low=512`, `medium=1024`,
...
1. Add provider metadata to [config/provider_catalog.py](src/claudey/config/provider_catalog.py).
2. Add credentials and related settings to [config/settings.py](src/claudey/config/settings.py)
...
3. Let Admin UI provider credential, configurable base URL, and proxy fields come from the
   ...
   [config/admin/](src/claudey/config/admin/) only when the generated manifest is
   insufficient.
4. Add an `OpenAIChatProfile` under [providers/openai_chat/](src/claudey/providers/openai_chat/) when
...
[src/claudey/core/anthropic/](src/claudey/core/anthropic/) owns Anthropic-side protocol behavior:
...
- stream lifecycle through `src/claudey/core/anthropic/streaming`, including the neutral
...
[src/claudey/core/anthropic/streaming/](src/claudey/core/anthropic/streaming/). The shared layer owns the
...
[core/failures.py](src/claudey/core/failures.py) defines the immutable,
...
[core/diagnostics.py](src/claudey/core/diagnostics.py) owns bounded error
...
[providers/failure_policy.py](src/claudey/providers/failure_policy.py)
...
[providers/stream_recovery.py](src/claudey/providers/stream_recovery.py)
...
execution budget. The OpenAI SDK's internal retries remain disabled so Claudey is
the only retry owner. `ExecutionFailure.retryable` records provider-policy
eligibility; it never tells the client to retry after Claudey has finalized the
failure.
...
[src/claudey/core/openai_responses/](src/claudey/core/openai_responses/) owns OpenAI Responses support:
...
- streaming-only `/v1/responses` support for Codex/Claudey workflows;
...
Claudey accepts omitted `stream` or `stream: true`; `stream: false` is rejected with
an OpenAI-shaped client error because installed Claudey/Codex workflows only need
...
entrypoint;
[src/claudey/core/openai_responses/streaming/](src/claudey/core/openai_responses/streaming/) owns the
...
Claudey does not validate custom-tool grammars.
...
`encrypted_content`; the opaque value is not exposed as visible text and Claudey
...
[api/optimization_handlers.py](src/claudey/api/optimization_handlers.py) short-circuits
...
[api/web_tools/](src/claudey/api/web_tools/). When `ENABLE_WEB_SERVER_TOOLS` is true, the
Messages handler can stream local Anthropic server-tool responses without sending the
request upstream. [api/web_tools/egress.py](src/claudey/api/web_tools/egress.py) enforces URL
...
[cli/local_http.py](src/claudey/cli/local_http.py) owns the direct
agent-to-Claudey connection boundary. Launcher health checks and local model-catalog
...
adds the configured Claudey host and standard loopback names to both `NO_PROXY` and
...
- After proxy health succeeds, it fetches `/v1/models`, writes a generated Codex
  `model_catalog_json` file under `~/.Claudey/`, and injects that path so Codex's
...
- Session commands load the extension from its absolute installed path and
  scope Pi to the ephemeral `claudey/**` provider, whose model IDs
  retain Claudey's nested `provider/model` routing reference.
- The extension fetches Claudey's `/v1/models` catalog before registration, projects
...
Codex and Pi are supported through their installed launchers. Claudey does not keep
...
Messaging is optional. [runtime/application.py](src/claudey/runtime/application.py) calls
`create_messaging_components()` from
[messaging/platforms/factory.py](src/claudey/messaging/platforms/factory.py) during startup.
...
[messaging/platforms/ports.py](src/claudey/messaging/platforms/ports.py): a
...
Runtime adapters in
[messaging/platforms/telegram.py](src/claudey/messaging/platforms/telegram.py) and
[messaging/platforms/discord.py](src/claudey/messaging/platforms/discord.py) own SDK client
...
[messaging/platforms/telegram_inbound.py](src/claudey/messaging/platforms/telegram_inbound.py)
and [messaging/platforms/discord_inbound.py](src/claudey/messaging/platforms/discord_inbound.py).
Outbound SDK calls live in
[messaging/platforms/telegram_io.py](src/claudey/messaging/platforms/telegram_io.py) and
[messaging/platforms/discord_io.py](src/claudey/messaging/platforms/discord_io.py). Shared
delivery policy lives in [messaging/platforms/outbox.py](src/claudey/messaging/platforms/outbox.py),
...
[messaging/platforms/voice_flow.py](src/claudey/messaging/platforms/voice_flow.py), which owns
...
in the `PendingVoiceRegistry` owned by [messaging/voice.py](src/claudey/messaging/voice.py).
...
[messaging/workflow.py](src/claudey/messaging/workflow.py) contains `MessagingWorkflow`, the
...
[messaging/turn_intake.py](src/claudey/messaging/turn_intake.py) owns slash command dispatch,
...
[messaging/node_runner.py](src/claudey/messaging/node_runner.py) owns managed CLI session
...
[messaging/event_parser.py](src/claudey/messaging/event_parser.py) normalizes managed Claude
JSON events into low-level transcript events.
[messaging/transcript/](src/claudey/messaging/transcript/) owns transcript assembly and
...
[messaging/rendering/](src/claudey/messaging/rendering/).

[messaging/command_context.py](src/claudey/messaging/command_context.py) defines the typed
...
[messaging/trees/runtime.py](src/claudey/messaging/trees/runtime.py) contains the
...
`parent_reference_id` records the exact prompt or Claudey status that received the
...
[messaging/trees/transitions.py](src/claudey/messaging/trees/transitions.py) owns frozen,
...
[messaging/trees/manager.py](src/claudey/messaging/trees/manager.py) is the only external
...
[messaging/trees/repository.py](src/claudey/messaging/trees/repository.py)
is manager-private and owns only aggregate/reference indexes.
[messaging/trees/processor.py](src/claudey/messaging/trees/processor.py) owns every
...
[messaging/trees/node.py](src/claudey/messaging/trees/node.py) owns
...
[messaging/trees/graph.py](src/claudey/messaging/trees/graph.py) owns parent/child and
status-message lookup state, and
[messaging/trees/snapshot.py](src/claudey/messaging/trees/snapshot.py) owns typed persisted
...
[messaging/session/](src/claudey/messaging/session/) persists typed conversation snapshots
...
[messaging/session/persistence.py](src/claudey/messaging/session/persistence.py). One writer
...
ownership lives in
[messaging/session/managed_message_log.py](src/claudey/messaging/session/managed_message_log.py).
The registry accepts managed inbound prompts, voice notes, and commands as well
as Claudey output. It migrates legacy `message_log` entries and persists the final
...
`/clear` commits Claudey state cleanup first and then best-effort deletes the exact
authorized message-ID set through the list-based outbound port. Standalone clear
deletes every tracked user and Claudey message in its chat; reply clear deletes only
...
missing messages; such failures never restore cleared Claudey state.
...
[core/trace.py](src/claudey/core/trace.py) emits structured trace events across stages such
...
1. Add or expose the setting in [config/settings.py](src/claudey/config/settings.py).
2. Add the template key to [.env.example](.env.example) if users configure it.
3. Add a `ConfigFieldSpec` under [config/admin/](src/claudey/config/admin/), or add
...
1. For an installed wrapper, add or update a launcher under
   [cli/launchers/](src/claudey/cli/launchers/) and keep credential stripping local to that
   client.
2. For messaging-managed execution, update [cli/managed/](src/claudey/cli/managed/) only
...
3. Ensure managed task parsing emits the event shapes expected by
   [messaging/event_parser.py](src/claudey/messaging/event_parser.py) and
   [messaging/node_event_pipeline.py](src/claudey/messaging/node_event_pipeline.py).
...
1. Implement a `MessagingRuntime`, `OutboundMessenger`, and inbound normalizer
   under [messaging/platforms/](src/claudey/messaging/platforms/).
2. Reuse [messaging/platforms/outbox.py](src/claudey/messaging/platforms/outbox.py) for
   queued outbound delivery and
   [messaging/platforms/voice_flow.py](src/claudey/messaging/platforms/voice_flow.py) for
   voice-note handoff when the platform supports audio.
3. Add construction logic to
   [messaging/platforms/factory.py](src/claudey/messaging/platforms/factory.py).
4. Add settings and admin fields for tokens, allowlists, and platform-specific
   runtime options.
5. Add rendering profile support in
   [messaging/rendering/profiles.py](src/claudey/messaging/rendering/profiles.py) if needed.
6. Add deterministic runtime/outbound/workflow tests and optional live smoke
   targets.

### Add Protocol Behavior

1. Put shared Anthropic behavior under [src/claudey/core/anthropic/](src/claudey/core/anthropic/).
2. Put OpenAI Responses behavior under
   [src/claudey/core/openai_responses/](src/claudey/core/openai_responses/).
