...
def test_pi_cli_prompt_e2e(smoke_config: SmokeConfig, tmp_path: Path) -> None:
    ...
    provider_model = ProviderMatrixDriver(smoke_config).first_model()
    auth_token = "claudey-pi-smoke-token"
    ...
    with SmokeServerDriver(
        ...
        result = subprocess.run(
            [
                ...
                "--no-approve",
                "--print",
                "Reply with exactly CLAUDEY_SMOKE_PI",
            ],
            cwd=tmp_path,
            env=env,
            check=False,
    ...
    assert "CLAUDEY_SMOKE_PI" in result.stdout
...
def test_claude_cli_adaptive_thinking_e2e(
    ...
    with SmokeServerDriver(
        ...
        result = ClientProtocolDriver.run_claude_prompt(
            ...
            config=smoke_config,
            cwd=tmp_path,
            prompt="think hard, then reply with exactly CLAUDEY_SMOKE_CLI",
        )
        server_log = server.log_path.read_text(encoding="utf-8", errors="replace")
    ...
    assert "CLAUDEY_SMOKE_CLI" in result.stdout
...
def test_claude_cli_provider_error_e2e(
    ...
    if not claude_bin:
        pytest.skip(f"missing_env: Claude CLI not found: {smoke_config.claude_bin}")
    broken_model = "lmstudio/claudey-smoke-failing-model"
    ...
    with (
        ...
        result = ClientProtocolDriver.run_claude_prompt(
            ...
            config=smoke_config,
            cwd=tmp_path,
            prompt="Reply with exactly CLAUDEY_SMOKE_UNREACHABLE.",
    ...
    assert "claudey smoke provider rejected the request deliberately" in lower
            ...
            if self.path == "/v1/models":
                self._write_json(
                    HTTPStatus.OK,
                    {
                        ...
                        "data": [
                            {
                                "id": "claudey-smoke-failing-model",
                                "object": "model",
                            }
                        ],
                    },
            ...
            self._write_json(
                HTTPStatus.BAD_REQUEST,
                {
                    "error": {
                        ...
                        "code": "claudey_smoke_failure",
                        "message": "CLAUDEY smoke provider rejected the request deliberately.",
                    }
                },
            )
