...
def test_telegram_live_permissions_e2e(smoke_config: SmokeConfig) -> None:
    ...
    if not chat_id:
        pytest.skip(
            "missing_env: HANS_SMOKE_TELEGRAM_CHAT_ID or "
            "ALLOWED_TELEGRAM_USER_ID required"
        )

    marker = f"HANS product smoke {int(time.time())}"
    sent = httpx.post(
...
def test_discord_live_permissions_e2e(smoke_config: SmokeConfig) -> None:
    ...
    if not channel_id:
        pytest.skip(
            "missing_env: HANS_SMOKE_DISCORD_CHANNEL_ID or "
...
def test_telegram_live_manual_inbound_e2e(smoke_config: SmokeConfig) -> None:
    if not smoke_config.interactive:
        pytest.skip(
            "missing_env: set HANS_SMOKE_INTERACTIVE=1 for manual inbound smoke"
        )
    pytest.skip(
