...
The second command skips everything unless `HANS_LIVE_SMOKE=1` is set, but still
writes skip entries to `.smoke-results/`.

## Product Smoke Run

```powershell
$env:HANS_LIVE_SMOKE = "1"
uv run pytest smoke -n 0 -s --tb=short
```
...
```powershell
$env:HANS_LIVE_SMOKE = "1"
$env:HANS_SMOKE_TARGETS = "providers"
...
catalog/docs and can be overridden with `HANS_SMOKE_MODEL_<PROVIDER>`, for example
`HANS_SMOKE_MODEL_DEEPSEEK=deepseek-v4-pro` (or `deepseek-v4-flash`). If no provider smoke model is
configured, live product smoke fails as `missing_env` unless you explicitly set
`HANS_ALLOW_NO_PROVIDER_SMOKE=1`.
...
| `providers` | multi-turn text, adaptive thinking history, tools, disconnect, errors | configured providers, optional `HANS_SMOKE_MODEL_*` |
...
| `voice` | generated WAV through local Whisper or NVIDIA NIM transcription | `VOICE_NOTE_ENABLED=true`, `HANS_SMOKE_RUN_VOICE=1` |

## Examples

```powershell
$env:HANS_LIVE_SMOKE = "1"
$env:HANS_SMOKE_PROVIDER_MATRIX = "open_router,nvidia_nim,deepseek,lmstudio,llamacpp,ollama"
uv run pytest smoke/product -n 0 -s --tb=short
```

```powershell
$env:HANS_LIVE_SMOKE = "1"
$env:HANS_SMOKE_TARGETS = "ollama"
...
```powershell
$env:HANS_LIVE_SMOKE = "1"
$env:HANS_SMOKE_TARGETS = "telegram,discord,voice"
$env:HANS_SMOKE_RUN_VOICE = "1"
uv run pytest smoke/product -n 0 -s --tb=short
```

```powershell
$env:HANS_LIVE_SMOKE = "1"
$env:HANS_SMOKE_TARGETS = "nvidia_nim_cli"
$env:HANS_SMOKE_NIM_MODELS = "z-ai/glm-5.2,moonshotai/kimi-k2.6,minimaxai/minimax-m2.7,minimaxai/minimax-m3,nvidia/nemotron-3-super-120b-a12b,deepseek-ai/deepseek-v4-pro,deepseek-ai/deepseek-v4-flash"
uv run pytest smoke/product -n 0 -s --tb=short
```

```powershell
$env:HANS_LIVE_SMOKE = "1"
$env:HANS_SMOKE_TARGETS = "openrouter_free_cli"
$env:HANS_SMOKE_OPENROUTER_FREE_MODELS = "nvidia/nemotron-3-super-120b-a12b:free,openai/gpt-oss-120b:free,poolside/laguna-m.1:free"
uv run pytest smoke/product -n 0 -s --tb=short
```

```powershell
$env:HANS_LIVE_SMOKE = "1"
$env:HANS_SMOKE_TARGETS = "messaging,config,extensibility"
uv run pytest smoke/product -n 0 -s --tb=short
```

## Environment

- `HANS_ENV_FILE`: explicit dotenv path for startup/config scenarios.
- `HANS_LIVE_SMOKE=1`: enables live smoke execution.
- `HANS_ALLOW_NO_PROVIDER_SMOKE=1`: permits no-provider live smoke for harness work.
- `HANS_SMOKE_TARGETS`: comma-separated targets, or `all`.
- `HANS_SMOKE_PROVIDER_MATRIX`: comma-separated provider prefixes to require.
- `HANS_SMOKE_MODEL_<PROVIDER>`: optional per-provider smoke model override.
  Use the uppercase provider ID, such as `HANS_SMOKE_MODEL_KILO`; the complete
...
- `HANS_SMOKE_MODEL_MISTRAL_REASONING`: optional override for the dedicated
  Mistral native reasoning smoke, default `mistral/mistral-medium-3-5`.
- `HANS_SMOKE_NIM_MODELS`: optional comma-separated NVIDIA NIM CLI matrix models
  that replace the default characterization set.
- `HANS_SMOKE_NIM_EXTRA_MODELS`: optional comma-separated NVIDIA NIM CLI matrix
  models appended to the default or replacement set.
- `HANS_SMOKE_OPENROUTER_FREE_MODELS`: optional comma-separated OpenRouter free
  CLI matrix models that replace the default characterization set.
- `HANS_SMOKE_OPENROUTER_FREE_EXTRA_MODELS`: optional comma-separated OpenRouter
  free CLI matrix models appended to the default or replacement set.
- `HANS_SMOKE_TIMEOUT_S`: per-request/subprocess timeout, default `45`.
- `HANS_SMOKE_CLAUDE_BIN`: Claude CLI executable name, default `claude`.
- `HANS_SMOKE_TELEGRAM_CHAT_ID`: Telegram chat/user ID for send/edit/delete.
- `HANS_SMOKE_DISCORD_CHANNEL_ID`: Discord channel ID for send/edit/delete.
- `HANS_SMOKE_INTERACTIVE=1`: enables manual inbound Telegram/Discord checks.
- `HANS_SMOKE_RUN_VOICE=1`: allows voice transcription backends to load/run.
...
`uv run`, so Windows does not try to replace `hans-server.exe` while it is
locked.
...
- `target_disabled`: skipped because `HANS_SMOKE_TARGETS` intentionally selected
  a different target.
...
explicitly selected a provider in `HANS_SMOKE_PROVIDER_MATRIX`;
