...
def pytest_collection_modifyitems(items: list[pytest.Item]) -> None:
    if SmokeConfig.load().live:
        return
    skip = pytest.mark.skip(reason="set HANS_LIVE_SMOKE=1 to run local smoke tests")
    for item in items:
...
def provider_model_params(config: SmokeConfig) -> list[Any]:
    ...
    if not config.live:
        return [_disabled_provider_param("set HANS_LIVE_SMOKE=1 to run provider smoke")]
